...
{
	const SDL_Rect& aDest = aIn.destination(); //When we move to SDL3, this'll be SDL_FRect
	const SDL_Rect& bDest = bIn.destination();
	return ! (aDest.x + aDest.w < bDest.x ||
			  aDest.y + aDest.h < bDest.y ||
			  aDest.x > bDest.x + bDest.w ||
		      aDest.y > bDest.y + bDest.h);
	//WSB 2025-04-12 test this as it's new logic not just syntax
	//This seemed more efficient than converting aDest, bDest to SDL_Rects expected by SDL_HasRectIntersection
}
