...
public:
	static SSDL_SoundSystem& Instance (int freq = 88020, Uint16 format = MIX_DEFAULT_FORMAT,
		...
		//This function's defaults are mentioned in C++/C++20/C++26 for Lazy Programmers, Appendix A,
		//   so don't dump them
	{
...
private:
	...
	{ 
		...
		//Here's why this is commented out: it may crash if compiled with g++ (SSDL_Display's Quit calls do)
...
{
public:
	inline static constexpr int NO_CHANNEL = -1;
	SSDL_Sound (Mix_Chunk* soundChunk = nullptr) : channel_ (NO_CHANNEL), chunk_ (nullptr)
	{
...
{
	if (!SSDL_SoundSystem::Instance().initialized()) return nullptr;  //If it won't initialize...
...
{
public:
	SSDL_Music (Mix_Music* m = nullptr)
	{
		SSDL_SoundSystem::Instance (); //so that it's created before I try to do anything with it
		setTo (m);
	}
	SSDL_Music(SSDL_Music&& other) noexcept { music_ = other.music_; other.music_ = nullptr; }
	...
	~SSDL_Music () { if (music_) Mix_FreeMusic (music_); music_ = nullptr; }
...
private:
	...
	{ 
		...
		//Why not just =delete?
		//  With g++, some implementations, if I delete the copy ctor, it complains that this is needed.  
	    //  In gdb, it never calls it.  For now, I'll throw this exception.  
	}
};

inline constexpr int SSDL_FOREVER = -1;
