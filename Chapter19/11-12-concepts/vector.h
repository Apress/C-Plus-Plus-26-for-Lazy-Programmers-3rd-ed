...
template<typename T>
concept VectorElement = std::assignable_from<T&, T> &&
...
template<typename T>
concept PrintableVectorElement = VectorElement<T> && Printable<T>;
...
public:
    ...
    void print(std::ostream& out) const requires Printable<T>;
private:
    T*          contents_;
    std::size_t howMany_;
...
template<VectorElement T>
void Vector<T>::print(std::ostream& out) const requires Printable<T>
{
    out << "[ "; for (unsigned int i = 0; i < size(); ++i) out << (*this)[i] << ' '; out << ']';
}
