                     ...
                     Deck& deck, Waste&  waste)
{
    ...
    while (!waste.empty()) waste.removeTop();
    for (Suit s = Suit::HEARTS; s <= Suit::SPADES; ++s)
        waste.addCardLegally(Card(Rank::ACE, s));
}
    ...
    do
    {
        ...
        else
        {
            ...
            try
            {
                ...
                if (inRange(emptyCellIndex) && cellAt(to).empty())
                    // if card to move exists, and move is legal...
                    if (!cellAt(from).empty() &&
                                                ...
                                                // for redeal
{
    ...
    while (! waste.empty()) deck.addCard(waste.removeTop());   
}
        ...
        for (unsigned int col = 0; col < COLS; ++col)
            if (grid_[row][col].empty())
            {
                if (whichEmptyCell >= NUM_EMPTY_CELLS)
                    throw std::out_of_range ("Too many empty cells!");
               ...
               // display needs to know whether to print A B C or D
{
    ...
    if (result == NOT_FOUND) throw std::out_of_range("Not a valid empty cell");
...
bool Montana::canGoHere(const Card& c, const GridLoc& loc) const
{
    ...
    if (grid_[loc.row_][loc.col_-1].empty())  return false;
