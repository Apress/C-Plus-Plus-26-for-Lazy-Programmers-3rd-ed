...
constexpr int HOIST                   = 390; //My pick for flag width
                                             //Called "A" in Fig. 9-1
constexpr int FLY                     = static_cast<int>(HOIST*1.9),   //B
              UNION_HOIST             = static_cast<int>(HOIST*0.5385),//C
              UNION_FLY               = static_cast<int>(HOIST*0.76);  //D                  

constexpr int UNION_VERTICAL_MARGIN   = static_cast<int>(HOIST*0.0538),//E&F
              UNION_HORIZONTAL_MARGIN = static_cast<int>(HOIST*0.0633);//G&H

constexpr int STAR_DIAMETER           = static_cast<int>(HOIST*0.0616);//K
...
void drawStar(int centerX, int centerY)
{
    ...
    // Find x, y point at current angle relative to center
    x1 = static_cast<int>(RADIUS * cos(angle));
    y1 = static_cast<int>(RADIUS * sin(angle));
    ...
    for (int i = 0; i < POINTS_ON_STAR; ++i)
    {
       angle += (2 * PI / 360) / POINTS_ON_STAR;
        ...
        // Find x, y point at new angle relative to center
       x2 = static_cast<int>(RADIUS * cos(angle));
       y2 = static_cast<int>(RADIUS * sin(angle));
