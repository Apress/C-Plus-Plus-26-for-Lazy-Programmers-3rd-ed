...
int main (int argc, char** argv)
{
    ...
    constexpr int PICTURE_WIDTH=500, PICTURE_HEIGHT=375; 
                             // size of the picture
    constexpr int WINDOW_WIDTH =500, WINDOW_HEIGHT =450; 

                            // size of entire window
                            // (has extra room for messages)
    SSDL_SetWindowTitle("Fossil hunt: a hidden-object game");
    ...
    // See where we clicked, and report if fossil was found
    ...
    // Is X between left side of box and right? 
    // Is Y also within bounds?
    ...
    if (isXInRange && isYInRange)
    {
        ...
        SSDL_RenderDrawRect(BOX_LEFT, BOX_TOP, 
