...
inline Rank operator+(Rank  r, int t) 
{
    return static_cast<Rank>(std::to_underlying(r) + t); 
}
inline Rank operator+=(Rank& r, int t) { return r = r + t;        }
inline Rank operator++(Rank& r)        { r += 1; return r;        }
...
class BadSuitException {};  // used if a Suit is out of range
    // ... you could also use a standard exception from #include <stdexcept>
...
inline bool operator==(const Card& a, const Card& b) noexcept
{
