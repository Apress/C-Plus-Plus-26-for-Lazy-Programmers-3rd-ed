...
inline int  SSDL_GetMouseClick (){ return SDL_GetMouseState (nullptr, nullptr);	   }
...
inline std::tuple<int, int>  SSDL_GetMouseLocation() { int x, y; SDL_GetMouseState(&x, &y); return std::tuple(x, y); }
...
// or (unless you turn off Escape meaning quit, below) if user hits Escape
int         SSDL_WaitMouse();
SDL_Keycode SSDL_WaitKey  (); //SDL_Keycode is SDL's virtual key rep. *Sometimes* casting to char gets you what you hit
void        SSDL_WaitEvent(Uint32 eventType, SDL_Event& event); //Note you can't OR eventTypes -- they're not distinct bits
