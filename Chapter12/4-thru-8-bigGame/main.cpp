...
constexpr int MARGIN             =  25; //As close to the L/R edges 
                                        // of screen as moving objects
                                        // are allowed to get
constexpr int BOTTOM_LINE        = 480; // Where last line of text goes
                                        // on instr & splash screens
...
const SSDL_Font SMALL_FONT = SSDL_OpenFont(
             "media/Sinister-Fonts_Werewolf-Moon/Werewolf Moon.ttf",
             SMALL_FONT_SIZE);
const SSDL_Font MEDIUM_FONT = SSDL_OpenFont(
             "media/Sinister-Fonts_Werewolf-Moon/Werewolf Moon.ttf", 
             MEDIUM_FONT_SIZE);
const SSDL_Font LARGE_FONT = SSDL_OpenFont(
             "media/Sinister-Fonts_Werewolf-Moon/Werewolf Moon.ttf", 
             LARGE_FONT_SIZE);
...
{
    // sound and picture
    static const SSDL_Sound VICTORY_SOUND = SSDL_LoadWAV(
            "media/342153__robcro6010__circus-theme-short.wav");
    SSDL_PlaySound(VICTORY_SOUND);

    static const SSDL_Image GOOD_PUMPKIN = SSDL_LoadImage(
            "media/goodPumpkin.png");
    ...
    SSDL_RenderText("Hooah!",
                    ...
                    SCREEN_HEIGHT/4, 
                    LARGE_FONT); 
    constexpr int LINE_DISTANCE_Y = 96; //arbitrarily chosen number...
...
{
    // sound and picture
    static const SSDL_Sound DEFEAT_SOUND = SSDL_LoadWAV(
              "media/326813__mrose6__echoed-screams-short.wav");
    SSDL_PlaySound(DEFEAT_SOUND);

    static const SSDL_Image SAD_PUMPKIN  = SSDL_LoadImage(
              "media/sadPumpkin.png");
...
void initializeObjects(Object& basket, Object& candy,
    Object& yumMessage)
{
    // load those images
...
void renderObjects(Object basket, Object candy,
    Object yumMessage, bool showYumMessage)
{
...
//  another candy...
{
    ...
    SSDL_SetSpriteLocation(candy.sprite_,
        candy.loc_.x_, candy.loc_.y_);
}
...
void moveObject(Object& object)
{
    ...
    SSDL_SetSpriteLocation(object.sprite_,
        object.loc_.x_, object.loc_.y_);
...
bool handleCatchingCandy (Object basket, Object& candy, 
                          Object& yumMessage, int& numberCaught)
{
    if (inCollision(basket.loc_, candy.loc_,
        CANDY_WIDTH, BASKET_WIDTH))
    {
...
bool playGame()
{
    ...
    while (SSDL_IsNextFrame() && !isVictory && !isDefeat)
    {
        ...
        constexpr int BASKET_SPEED = 7;//7 pixels per frame, L or R
        moveBasket(basket, BASKET_SPEED);
        moveObject(candy); moveObject(yumMessage);

        // Did you catch a candy?
        if (handleCatchingCandy(basket, candy,
            yumMessage, numberCaught))
